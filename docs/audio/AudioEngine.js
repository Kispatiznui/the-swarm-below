...
class AudioEngine {
    ...
    /* Called once per game frame from Game.update(). Derives
       ...
       rampSeconds is optional and only meant for the debug
       tool: normal gameplay never passes it, so it keeps the
       default musical ramp (see AudioAmbience.setIntensity).
       DebugMode passes a short ramp so jumping straight into
       wave 30 is audible immediately instead of fading in
       over several seconds. */

    update(state, rampSeconds) {
        ...
        this.setIntensity(intensity, rampSeconds);
    }
